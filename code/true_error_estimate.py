# -*- coding: utf-8 -*-
"""
Created on Sun Mar  1 18:38:48 2020
...
#        X_ap, X_test, y_ap, y_test = train_test_split(X, y, test_size = 0.2, random_state = rand_state, stratify = y)
        ...
        model = clone(model)
#        pred, prob, _ = test_model(model, X, X_holdout, y)
...
#    df = prc.standarize(df) # or normalize
    ...
    dt = AdaBoostClassifier(DecisionTreeClassifier(max_depth=1, class_weight = "balanced"), n_estimators=20)
    print(main(optimal_features, "AdaBoost", dt, bs_estimate = True, verbose=True))
